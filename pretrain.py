...
def train(params, base_loader, val_loader, model, stop_epoch):
    ...
    if params.method in ['stl_deepbdc', 'meta_deepbdc']:
        ...
        optimizer = torch.optim.SGD([
            {'params': bas_params},
            {'params': model.dcov.temperature, 'lr': params.t_lr}], lr=params.lr, weight_decay=params.wd, nesterov=True,
            momentum=0.9)
    else:
        optimizer = torch.optim.SGD(model.parameters(), lr=params.lr, weight_decay=5e-4, nesterov=True, momentum=0.9)
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer=optimizer, milestones=params.milestones,
                                                        gamma=params.gamma)
        ...
        if params.val in ['meta']:
            ...
            if acc > trlog['max_acc']:
                ...
                outfile = os.path.join(params.checkpoint_dir,
                                       'checkpoints/cub/ResNet12_meta_deepbdc_5way_5shot_metatrain/best_model.tar')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--t_lr', type=float, default=0.05,
                        help='initial learning rate uesd for the temperature of bdc module')
    ...
    parser.add_argument('--dataset', default='cub', choices=['mini_imagenet', 'tiered_imagenet', 'cub'])
    parser.add_argument('--data_path', type=str,default=r'C:\Users\Lenovo\Desktop\机器学习\内镜\DeepBDC-main\filelist\cub', help='dataset path')

    parser.add_argument('--model', default='ResNet12', choices=['ResNet12', 'ResNet18'])
    parser.add_argument('--method', default='meta_deepbdc',
    ...
    parser.add_argument('--n_shot', default=1, type=int,
                        help='number of labeled data in each class, same as n_support during meta validation')
