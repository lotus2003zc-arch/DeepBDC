...
class MetaDeepBDC(MetaTemplate):
    ...
    def get_proto(self, support):
        support = support.contiguous().view(self.n_way * self.n_support, *support.size()[2:])
        support = self.feature.forward(support)
        z_all = self.feature_forward(support)
        z_all = z_all.view(self.n_way, self.n_support, -1)
        z_proto = z_all.contiguous().view(self.n_way, self.n_support, -1).mean(1)
        return z_proto

    def pred(self, x, support):
        proto = self.get_proto(support)
        x = self.feature.forward(x)
        x = self.feature_forward(x)
        scores = self.metric(x, proto)
        return scores
